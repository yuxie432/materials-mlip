"""Retrofit net magnetic moment + net charge onto an already-built dataset (shard-rewriting).

The first harvests stored neither the net moment nor the net charge, and stored per-atom
``dft_magmom``/``dft_charge`` on the final frame of vasprun+OUTCAR calcs. This module brings an
existing dataset to the new schema: every frame gains ``total_magnetization`` /
``total_charge`` (:mod:`electronic`), the per-atom arrays are stripped, and each metadata
record gains the ``electronic`` block (``site_*_present`` removed). Unlike the OUTCAR / vasprun
/ availability recoveries (metadata-only, shards untouched), this one **rewrites shards** — the
values live on the frames — so it is deliberately a separate, clearly-labelled tool.
...
1. :func:`compute_net_properties` (Phase 1, disk-paced) — re-fetch the records (the ordinary
   ...
   a fresh parse uses) into a ``net_properties.jsonl`` map (``calc_id`` → block). Resumable:
   calcs already in the map are skipped, so the fetch→compute→purge loop can pace against the
   disk quota exactly like the other recoveries. No shard is touched here.
2. :func:`apply_net_properties` (Phase 2, local, no network) — drive the map into the dataset:
   a **text-level** shard edit (append the two info keys to each frame's comment line, strip the
   ``dft_magmom``/``dft_charge`` columns) that leaves every untouched frame — and every kept
   value — **byte-identical** (no ASE re-serialisation, so no float-precision drift), then an
   atomic metadata rewrite adding ``electronic`` and dropping ``site_*_present``. Idempotent
   (re-running strips-then-re-appends the keys and re-strips absent columns → no-op) and
   resumable (a ``.net_properties_applied`` marker skips shards already done).
...
block change. The design is intentionally compatible with a later per-frame addition (e.g. an
OUTCAR ionic-convergence ΔE): another Phase-1 field + the same Phase-2 text edit.
"""
...
def compute_net_properties(fetched: str | Path, raw_dir: str | Path,
    ...
    """
    from .parse import _calc_id, _resolve, electronic_block_for_unit
    ...
    if dataset_dir is not None:
        ...
        dataset_calc_ids = {r["calc_id"] for r in read_jsonl(mp) if r.get("calc_id")} if mp.is_file() else set()
        ...
        for rec in read_jsonl(fetched):
            ...
            for unit in rec.get("calc_units", []):
                ...
                if not primary or not os.path.exists(primary):
                    ...
                    continue
                block = electronic_block_for_unit(unit_abs)
                out.write({"calc_id": calc_id, "electronic": block})
                done.add(calc_id)
                n_computed += 1
    return {
        "ok": True,
...
def _rewrite_shard_text(shard: Path, blocks: dict[str, dict]) -> dict:
    """Text-edit one shard's frames in place (atomic). Strips per-atom dft columns from every
    frame that has them and appends the two totals to frames whose calc_id is in ``blocks``.

    Reads the whole (intact) shard text and walks extxyz frames. If any frame is malformed the
    shard is left UNTOUCHED (returns ``ok=False``) rather than risk corrupting it — safer than a
    partial rewrite, and the recovery can be re-run. Only rewrites when something changed.
    """
    try:
    ...
    while i < len(lines):
        ...
        appended = False
        if calc_id in blocks:
            new_comment = _append_totals(comment2, blocks[calc_id])
...
def apply_net_properties(dataset_dir: str | Path, net_properties: str | Path,
    ...
    to add ``total_magnetization``/``total_charge`` per frame and strip the per-atom dft columns,
    then rewrites ``metadata.jsonl`` (atomic, one-time ``metadata.jsonl.bak.pre_net_properties``
    backup) to add each calc's ``electronic`` block and drop ``site_*_present``. Held under the
    dataset ``.parse.lock``. ``dry_run`` reports what would change and writes nothing.
    """
    dataset_dir = Path(dataset_dir)
    metadata_path = dataset_dir / "metadata.jsonl"
    if not metadata_path.is_file():
    ...
    stats: dict[str, Any] = {"ok": True, "dataset_dir": str(dataset_dir), "dry_run": dry_run,
                             "map_size": len(blocks), "shards_total": 0, "shards_rewritten": 0,
    ...
    if dry_run:
        ...
        for record in read_jsonl(metadata_path):
            ...
            if record.get("calc_id") in blocks:
        ...
        with marker.open("a") as mk:
            for shard in shards:
                if shard.name in applied:
                    ...
                    continue
                res = _rewrite_shard_text(shard, blocks)
                if not res["ok"]:
