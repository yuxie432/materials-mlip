...
#SBATCH -o logs/nomad-discover-%j.out
#SBATCH -e logs/nomad-discover-%j.err
...
# ---- ENV SETUP (edit me) --------------------------------------------------------
# ZENODO_HARVEST_DATA is read at IMPORT time, so it must be set before python starts. NOMAD
# reuses the same scratch root + the shared config layout as the Zenodo harvest.
