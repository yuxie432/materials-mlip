...
# the same quota (est. 15-75 GB), and the valve's bounded overshoot. NB the valve books
# 4x each download (measured extraction expansion), so it stops well before this in bytes
...
# pymatgen holds a whole ionic trajectory in RAM, so one huge vasprun.xml can get the
# job cgroup-killed, losing the run's progress. Start here, then check the real peak
# with `sacct -j <jobid> --format=MaxRSS` and raise it if there is headroom.
